...
repeat(ds_height){
	...
	c  = c_white;
	xo = 0;
	if(yy == menu_option[pages]) {
		c = c_blue;
		xo = -(x_buffer/2);
		//Dibujar el puntero
		var stringLen = string_width(ds_grid[# 0, yy]);
		draw_sprite_ext(sprMenuPointer, 0, (ltx + xo) - stringLen - 40, lty, 0.2, 0.2, 0, -1, 1);
	}
...
repeat(ds_height){
	...
	switch(ds_grid[# 1, yy]){
		case menu_element_type.shift:
			...
			if(inputting and yy == menu_option[pages]) {
				c = c_blue;
			}
		...
		case menu_element_type.input:
			...
			switch(current_val){
				...
				case vk_escape:	string_val = "ESCAPE KEY"; break;
